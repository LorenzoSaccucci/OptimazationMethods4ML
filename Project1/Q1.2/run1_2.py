...
import numpy as np
import time
from functions1_2 import rbf_pred, E2, grad_E_rbf, fun_plot3, multistart_grid_search_custom_RBF 
...
sigma = 0.9  #0.95 1 0.9 #1
print("The value of the spread of the activation function is: ", sigma)
rho = 0.00001
print("The values of the regularization terms are both: ", rho)
...
#parameters random initialization (with normal distribution)
np.random.seed(9027) #9027 22
omega = np.random.randn(N*(n+m)) 
...
y_hat_i = rbf_pred(omega, N, sigma, X_train)
#print("Initial prediction:", y_hat_i)

#initial error with random parameters
e_i = mean_squared_error(y_train, y_hat_i)*0.5
print("Initial MSE :",e_i)

#print("Initial omega squared-norm:", (np.sum(omega**2)))

#initial regularized error
E_i = E2(omega, N, sigma, rho, X_train, y_train)
print("Initial Regularized Error:",E_i)

#initial gradient norm
grad_E_i = grad_E_rbf(omega, N, sigma, rho, X_train, y_train)
#print("Initial Gradient:", grad_E_i)
print("Initial Gradient Norm:", np.linalg.norm(grad_E_i))

#check gradient e.g. in the initial guess
grad_error1 = check_grad(E2, grad_E_rbf, omega, N, sigma, rho, X_train, y_train)
...
result = minimize(E2, omega, args = (N, sigma, rho, X_train, y_train), jac = grad_E_rbf, method='BFGS', options={'gtol': 1e-5}) #tol=1e-6 
...
rho1,rho2 = rho, rho 
#print("Regularization term:", 0.5*(rho1*np.sum(omega_opt[:N]**2) + rho2*np.sum(omega_opt[N:]**2)))



#final gradient norm
grad_E_opt = grad_E_rbf(omega_opt, N, sigma, rho, X_train, y_train)
print("Final Gradient Norm (Train):", np.linalg.norm(grad_E_opt))
print()
#gradient evaluation in the optimal
grad_error_opt = check_grad(E2, grad_E_rbf, omega_opt, N, sigma, rho, X_train, y_train)
#print('Gradient error in the optimal point:', grad_error_opt)

#prediction on train based on optimal parameters
y_hat_opt = rbf_pred(omega_opt, N, sigma, X_train)
...
#RBF:
#results, best_params, best_seed= multistart_grid_search_custom_RBF(X_train, y_train, possible_N, possible_sigma, possible_rho, K, 10)
#print(results)
...
#results.to_excel('risultati_grid_rbf.xlsx', index=False)
