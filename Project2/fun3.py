...
def K(X, Y, gamma, kernel):
    if kernel == 'Polynomial':
        ...
        # return np.power(1 + np.dot(X, Y.T), gamma)
...
def calculate_t_max(d, alpha_component, C):
    if d > 0 and alpha_component < C:
        return (C - alpha_component) / d
    elif d < 0 and alpha_component > 0:
...
def SMO(X, y, ker, gamma, C, tol):
    ...
    while m > M:
        ...
        M = min((-grad * y)[S_set])
        k += 1
    print('KKT violation (m - M) equal to:', m-M)
    return alpha, k
